...
{                          
    ...
    public function UpdateRelatedProducts()
    {
		...
		$this->ProductsCount = $this->GetProductsCount();//кол-во товаров всего
		...
		foreach($WorkProducts as $WorkProduct)
		{
			...
			$RandomCount = self::MaxRelatedProdsCount - $CountExist;//сколько нужно случайных
            ...
            $this->AddProductRelationsToDB($WorkProduct,$AddProductIds);//добавляем в базу
		}
    }
